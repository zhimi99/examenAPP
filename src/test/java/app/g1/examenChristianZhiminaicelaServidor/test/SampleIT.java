package app.g1.examenChristianZhiminaicelaServidor.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/examenChristianZhiminaicelaServidor.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "examenChristianZhiminaicelaServidor.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("app.g1.examenChristianZhiminaicelaServidor.test");
